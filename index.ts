...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    registerGatewayMethod("a2a.metrics", (opts) => {
      opts.respond(true, {
        metrics: telemetry.snapshot(),
      });
    });

    registerGatewayMethod("a2a.audit", (opts) => {
    ...
    registerGatewayMethod("a2a.pushNotification.register", (opts) => {
      const payload = asObject(opts.params);
    ...
    registerGatewayMethod("a2a.pushNotification.unregister", (opts) => {
      const payload = asObject(opts.params);
      const taskId = asString(payload.taskId, "");
      if (!taskId) {
    ...
    registerGatewayMethod("a2a.send", (opts) => {
      const payload = asObject(opts.params);
